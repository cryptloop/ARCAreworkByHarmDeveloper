...
(function(){
  ...
  /* ---------- Forms (Web3Forms) ---------- */
  // README: получите бесплатный ключ на https://web3forms.com (придёт на почту)
  // и вставьте его в оба скрытых поля <input name="access_key"> в index.html
  async function handleSubmit(form){
    form.addEventListener('submit', async e=>{
      ...
      // Проверка, что ключ заменён на реальный
      const keyInput = form.querySelector('[name="access_key"]');
      if(!keyInput || keyInput.value === 'YOUR_ACCESS_KEY_HERE'){
        showToast('⚠ Ключ доступа не настроен — см. README. Заявка не отправлена.');
        return;
      }
      ...
      try{
        ...
        showToast('✗ Не удалось отправить. Позвоните +7 (343) 200-00-00');
